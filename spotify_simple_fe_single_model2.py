...
# 引数に params を追加
def train_single_model(X, X_test, y, params):
    folds = KFold(n_splits=N_SPLITS, shuffle=True, random_state=SEED)
    oof = np.zeros(len(X))
    test_pred = np.zeros(len(X_test))

    for fold, (train_idx, valid_idx) in enumerate(folds.split(X), start=1):
        # 決め打ちではなく、渡された params を展開して適用する
        # foldごとに乱数シードを変えたい場合はここで上書きします
        current_params = params.copy()
        current_params["random_state"] = SEED + fold
        
        model = ExtraTreesRegressor(**current_params)
...
def objective(trial, X, y):
    ...
    # 2. 提案されたパラメータでCV（交差検証）を実行
    for train_idx, valid_idx in folds.split(X):
        model = ExtraTreesRegressor(**params)
        model.fit(X.iloc[train_idx], y[train_idx])
        oof[valid_idx] = model.predict(X.iloc[valid_idx])

    # 3. 最小化したい評価指標（今回はMSE）を返す
    score = mean_squared_error(y, oof)
    return score
...
def main():
    ...
    # 'minimize' = 戻り値(MSE)を最小化する方向で探索する
    study = optuna.create_study(direction="minimize")
    
    # n_trials=30 の場合、30パターンの組み合わせを試す（計算時間に合わせて調整してください）
    study.optimize(lambda trial: objective(trial, X, y), n_trials=30)
    
    print("Best params found by Optuna:")
    print(study.best_params)
    print("Best OOF MSE:", study.best_value)
    # ----------------------------------------

    # 探索で見つけた最適なパラメータを取り出し、固定パラメータ（乱数シードなど）を追加
    best_params = study.best_params
    best_params["random_state"] = SEED
    best_params["n_jobs"] = -1

    # 見つかった最適パラメータを train_single_model に渡して本番の学習を実行
    oof, test_pred = train_single_model(X, X_test, y, best_params)
    
    # リーク（track_idの一致）を利用した強力な後処理
